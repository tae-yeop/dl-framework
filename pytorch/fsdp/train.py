...
def train(args, model, rank, world_size, train_loader, optimizer, epoch, sampler=None):
    model.train()
    ddp_loss = torch.zeros(2).to(rank) # <- 이건 왜 있는건지?
    if sampler:
...
# fsdp 모델을 저장하기 위해선 각 rank에 대해서 state_dict를 call
# 전채 states를 Rank0에서 저장
# 
